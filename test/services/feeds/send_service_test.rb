...
class Feeds::SendServiceTest < ActiveSupport::TestCase
  ...
  test '赞一条评论，发送 feed' do
    assert_difference -> { comments(:three).author.feeds_count.value } do
  ...
  test '赞一条评论，评论作者是点赞者本人，不发送 feed' do
    assert_no_difference -> { comments(:one).author.feeds_count.value } do
      perform_enqueued_jobs do
  ...
  test '赞一条评论，评论作者关闭通知，不发送 feed' do
    ...
    assert_no_difference -> { comments(:three).author.feeds_count.value } do
      perform_enqueued_jobs do
        Feeds::SendService.create(@like_comment_params)
      end
    end
  ...
  test '赞一条主题，主题作者关闭通知，不发送 feed' do
    users(:yuki).membership_by_forum(forums(:one)).preference.update(feed_allowed: false)
    assert_no_difference -> { posts(:three).author.feeds_count.value } do
      perform_enqueued_jobs { Feeds::SendService.create(@like_post_params) }
    end
  end

  test '评论一条主题，发送 feed' do
    assert_difference -> { comments(:three).commentable.author.feeds_count.value } do
      perform_enqueued_jobs { Feeds::SendService.create(@comment_params) }
    end
  end

  test '评论一条主题，主题作者是评论者本人，不发送 feed' do
    assert_no_difference -> { comments(:one).commentable.author.feeds_count.value } do
      perform_enqueued_jobs do
  ...
  test '评论一条主题，主题作者关闭通知，不发送 feed' do
    users(:yuki).membership_by_forum(forums(:one)).preference.update(feed_allowed: false)
    assert_no_difference -> { comments(:three).commentable.author.feeds_count.value } do
      perform_enqueued_jobs { Feeds::SendService.create(@comment_params) }
    end
  end

  test '回复提及某人，发送 feed' do
    assert_difference -> { comments(:five).replied_user.feeds_count.value } do
      perform_enqueued_jobs do
  ...
  test '回复提及自己，不发送 feed' do
    assert_no_difference -> { comments(:four).replied_user.feeds_count.value } do
      perform_enqueued_jobs do
  ...
  test '回复提及题主，发送 feed' do
    assert_difference -> { comments(:four).replied_user.feeds_count.value } do
      perform_enqueued_jobs do
  ...
  test '发布新主题，给作者之外的其它圈子成员，发送 feed，圈子成员关闭通知不给该成员发送 feed' do
    users(:yuki).membership_by_forum(forums(:one)).preference.update(feed_allowed: false)
    assert_no_difference -> { users(:yuki).feeds_count.value } do
      no_feed_to_author_after_post
    end
  end
